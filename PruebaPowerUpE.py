...
try:
    ...
    shield_hit_sound = pygame.mixer.Sound(os.path.join(os.path.dirname(__file__), "shield_hit.wav")) 
    double_score_activate_sound = pygame.mixer.Sound(os.path.join(os.path.dirname(__file__), "double_score.wav")) 
except Exception as e:
...
# JUEGO ESTÁNDAR
# ========================
# --- NUEVO: Constantes de Power-Ups Globales (MOVIMIENTO CLAVE) ---
...
def jugar(nombre_fuente, tam, color, num_jugadores=2, initial_speed=1.5, count_wrong_key_faults=False, time_limit_seconds=0, fallos_limit=10, initial_state=None, save_timestamp=None):
    ...
    if num_jugadores == 1:
        ...
        if initial_state:
            ...
            pulsaciones_correctas = initial_state.get("pulsaciones_correctas", 0)
            if "letras_restantes_ciclo" in initial_state:
                letras_restantes_ciclo = initial_state["letras_restantes_ciclo"]
            ...
            jugadores = {"J1": {"letra_actual": obtener_nueva_letra(), 
                                "x": random.randint(0, ANCHO - 50), "y": 0, "score": 0, 
                                "fallos": 0, "color": color, "anim_offset": random.uniform(0, 2 * math.pi)}}
        ...
        if initial_state:
            ...
            active_letter = initial_state.get("active_letter", "") 
            ...
            if not active_letter: active_letter = obtener_nueva_letra()
        else:
            current_turn_player = "J1"
    ...
    while run:
            ...
            if btn_pausa.handle_event(evento):
                ...
                if accion_pausa == "guardar_y_salir":
                    estado_actual = {
                        ...
                        "pulsaciones_correctas": pulsaciones_correctas, 
                        "J1": jugadores["J1"],
                ...
                # Manejo de letras pulsadas
                    ...
                    if num_jugadores == 1:
                        ...
                        if typed_letter == j1["letra_actual"]:
                            ...
                            j1["score"] += (puntos_base + racha_actual) 
                            racha_actual += 1
                            crear_particulas(j1["x"] + 25, j1["y"] + 25, j1["color"])
                            j1["letra_actual"] = obtener_nueva_letra() 
                            margen_seguro = tam
                            ...
                            # Si es un múltiplo de 10 aciertos Y NO hay power-ups activos, genera uno
                            if pulsaciones_correctas > 0 and pulsaciones_correctas % 10 == 0:
                                if not power_up_manager.activos: # Solo genera si NO hay power-ups activos en este momento
                                    ...
                                    # Reproduce el sonido de activación correcto
                                    if power_up_a_generar == "escudo" and powerup_activate_sound: # Cambiado a powerup_activate_sound
                                        powerup_activate_sound.play()
                                    elif power_up_a_generar == "doble_puntuacion" and double_score_activate_sound:
                                        double_score_activate_sound.play()
                                    elif power_up_a_generar == "ralentizar" and powerup_activate_sound: # Cambiado a powerup_activate_sound
                                        powerup_activate_sound.play() 
                                    
                                    # Si activas "ralentizar" aquí, aplica el efecto inmediatamente
                                    if power_up_a_generar == "ralentizar":
                                        velocidad /= 2 
                                else:
                                    # print("No se generó Power-Up: Ya hay uno activo.") # Mensaje de depuración
                                    pass # No se hace nada si ya hay uno activo
                                
                        else: # El jugador 1 pulsa una letra incorrecta
                            if count_wrong_key_faults: 
                                if power_up_manager.esta_activo("escudo") and ShieldPowerUp().absorber_golpe(): 
                    ...
                    # Lógica para MODO 2 JUGADORES (VERSUS)
                        ...
                        if typed_letter == active_letter:
                            ...
                            if power_up_manager.esta_activo("doble_puntuacion"):
                                puntos_base *= 2 
                            jugadores[current_turn_player]["score"] += puntos_base 
        ...
        # --- Actualizar TODOS los power-ups con el manager ---
        terminados_en_este_ciclo = power_up_manager.actualizar()
        
        # Procesar power-ups que terminaron
        ...
        if num_jugadores == 1:
            letra_x = j1["x"] + desplazamiento_x
            letra_y = j1["y"]
            letra_texto = j1["letra_actual"]
        else:
    ...
    if num_jugadores == 1:
        if check_if_highscore(jugadores['J1']['score']):
            pantalla_ingresar_nombre(jugadores['J1']['score'])
            pantalla_highscores() 
            return "menu_principal" 
        else:
